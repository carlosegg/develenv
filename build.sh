...
function get_external(){
  ...
  local dependency_name=$(basename $external_url|cut -d'.' -f1)
  pushd . >/dev/null
...
function get_externals(){
  ...
  rsync --delete -arv $EXT_DEVELENV_DIR/screenshot/src/main/rpm/SOURCES/*.tar.gz screenshot/src/main/rpm/SOURCES
